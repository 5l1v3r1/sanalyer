...
function get_user_info_func($xmlrpc_params)
{
    ...
    $is_ignored = false;
    if(isset($visitor['ignored']) && !empty($visitor['ignored']))
    {
        $ignore_users = array_keys(unserialize($visitor['ignored']));
        $is_ignored = in_array($user['user_id'],$ignore_users);
    }
    $xmlrpc_user_info = new xmlrpcval(array(
        ...
        'is_ignored'         => new xmlrpcval($is_ignored, 'boolean')
    ), 'struct');
