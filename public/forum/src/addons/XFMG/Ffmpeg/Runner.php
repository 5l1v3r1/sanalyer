...
class Runner
{
	...
	public function getKeyFrame()
	{
		...
		{
			// Get a frame from somewhere in the first fifth of the video as seeking a large video can take time.
			$duration = $this->getVideoDuration();
			$seek = intval(round($duration / 5));

			$this->run("-ss {seek} -i {input} -vf thumbnail=50 -vsync 0 -f image2pipe -vcodec png -frames:v 1 -y {output}", [
				'seek' => $seek,
				'input' => $this->fileName,
				'output' => $newTempFile
			]);
		...
		{
			$this->run("-i {input} -an -f image2pipe -vcodec png -y {output}", [
				'input' => $this->fileName,
				'output' => $newTempFile
			]);
		}
	...
	public function supportsThumbnailFilter()
	{
		...
		foreach ($output AS $line)
		{
			...
			else if (preg_match("/(?:[a-z]|\.){3}\s(thumbnail)\b/m", $line))
			{
				return true;
			}
		}
