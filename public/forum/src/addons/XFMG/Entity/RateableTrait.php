...
trait RateableTrait
{
	...
	protected function _postDeleteRatings()
	{
		...
		$ratingIds = $this->db()->fetchAllColumn("
			SELECT rating_id
			FROM xf_mg_rating
			WHERE content_type = ?
				AND content_id = ?
	...
	{
		$structure->columns['rating_count'] = ['type' => self::UINT, 'forced' => true, 'default' => 0];
		$structure->columns['rating_sum'] = ['type' => self::UINT, 'forced' => true, 'default' => 0];
		$structure->columns['rating_avg'] = ['type' => self::FLOAT, 'default' => 0];
		$structure->columns['rating_weighted'] = ['type' => self::FLOAT, 'default' => 0];
