...
class Media extends AbstractController
{
	...
	public function actionView(ParameterBag $params)
	{
		...
		{
			$extraWith = [
				'DraftComments|' . $visitor->user_id,
				'CommentRead|' . $visitor->user_id,
				'Viewed|' . $visitor->user_id
			];
		}
	...
	protected function setupNoter(\XFMG\Entity\MediaItem $mediaItem, \XFMG\Entity\MediaNote $note)
	{
		...
		else if (!$taggedUsername && !$noteText)
		{
	...
	public function actionAdd()
	{
		...
		$albumList = $albumRepo->findAlbumsUserCanAddTo();

		$canAddToAlbums = ($albumList->total() > 0);
	...
	{
		$mediaItem = $this->assertViewableMediaItem($params->media_id);
		if (!$mediaItem->canUndelete($error))
		{
			return $this->noPermission($error);
		}
		...
		if ($this->isPost())
		{
			if ($mediaItem->media_state == 'deleted')
			{
		...
		else
		{
			...
			return $this->view('XFMG:Media\Delete', 'xfmg_media_undelete', $viewParams);
		}
	}
	...
	public function actionLike(ParameterBag $params)
	{
		...
		return $likePlugin->actionToggleLike(
			$mediaItem,
			$this->buildLink('media/like', $mediaItem),
			$this->buildLink('media', $mediaItem),
			$this->buildLink('media/likes', $mediaItem)
		);
	}
	...
	public function actionLikes(ParameterBag $params)
	{
		...
		$breadcrumbs = $mediaItem->getBreadcrumbs();
		$title = \XF::phrase('xfmg_members_who_liked_media', ['title' => $mediaItem->title]);
	...
	public function actionSetAsAvatar(ParameterBag $params)
	{
		...
		$tempFile = \XF\Util\File::copyAbstractedPathToTempFile($mediaItem->getAbstractedDataPath());
