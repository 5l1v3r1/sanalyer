...
class Comment extends AbstractController
{
	...
	public function actionLike(ParameterBag $params)
	{
		...
		return $likePlugin->actionToggleLike(
			$comment,
			$this->buildLink('media/comments/like', $comment),
			$this->buildLink('media/comments', $comment),
			$this->buildLink('media/comments/likes', $comment)
		);
	}
	...
	public function actionLikes(ParameterBag $params)
	{
		...
		$breadcrumbs = $comment->Content->getBreadcrumbs();
		$title = \XF::phrase('xfmg_members_who_have_liked_comment_by_x', ['user' => $comment->username]);

		/** @var \XF\ControllerPlugin\Like $likePlugin */
		$likePlugin = $this->plugin('XF:Like');
		return $likePlugin->actionLikes(
			$comment,
			['media/comments/likes', $comment],
			$title, $breadcrumbs
		);
	}
