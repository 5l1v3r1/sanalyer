...
class Media extends Repository
{
	...
	public function getMediaTypeFromExtension($extension)
	{
		...
		$imageExtensions = preg_split('/\s+/', trim($options->xfmgImageExtensions), -1, PREG_SPLIT_NO_EMPTY);
		$videoExtensions = preg_split('/\s+/', trim($options->xfmgVideoExtensions), -1, PREG_SPLIT_NO_EMPTY);
		$audioExtensions = preg_split('/\s+/', trim($options->xfmgAudioExtensions), -1, PREG_SPLIT_NO_EMPTY);
	...
	public function generateRandomMediaCache()
	{
		...
		while ($iterations > 0)
		{
			...
			$gt = $randFunction(0, max(0, $maxId - $limit));
	...
	public function rebuildImageThumb(MediaItem $mediaItem, $log = true)
	{
		...
		{
			if (!$this->app()->fs()->has($mediaItem->getAbstractedDataPath()))
			{
				return false;
			}
	...
	public function rebuildFFmpegThumb(MediaItem $mediaItem, $log = true)
	{
		...
		try
		{
			$tempThumb = $generatorService->getTempThumbnailFromFfmpeg($tempFile, $thumbPath, $mediaItem->media_type);
			if (!$tempThumb)
			{
				return false;
			}
