...
class Brivium_Store_ControllerPublic_Store extends XenForo_ControllerPublic_Abstract
{
	...
	public function actionProductPurchaseConfiguration()
	{
		...
		$product = XenForo_Application::get('brsProducts')->get($purchasedRecord['product_id']);
	...
	public function actionRemoveProductPurchase()
	{
		...
		$product = XenForo_Application::get('brsProducts')->get($active['product_id']);
	...
	public function actionCurrentPurchaseSave()
	{
		...
		$userId = XenForo_Visitor::getUserId();
		$productType = XenForo_Application::get('brsProductTypes')->get($productTypeId);
	...
	public function actionPurchased()
	{
		...
		foreach($productActives as $purchase){
			$products[$purchase['product_id']] = XenForo_Application::get('brsProducts')->get($purchase['product_id']);
