...
class XenGallery_ControllerPublic_Category extends XenGallery_ControllerPublic_Abstract
{
	...
	{
		$categoryId = $this->_input->filterSingle('category_id', XenForo_Input::UINT);
		$markCategoryId = $this->_input->filterSingle('mark_category_id', XenForo_Input::UINT);
		...
		if ($this->isConfirmedPost())
		{
			...
			if ($markCategoryId)
			{
				$fetchOptions['category_id'] = $markCategoryId;
			}

			$mediaIds = $mediaModel->getUnviewedMediaIds($visitor->user_id, $fetchOptions);
