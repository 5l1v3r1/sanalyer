...
class XenGallery_ControllerPublic_Album extends XenGallery_ControllerPublic_Abstract
{
	...
	public function actionEdit()
	{
		...
		if ($this->isConfirmedPost())
		{
			...
			$this->_sendAuthorAlert($album, 'xengallery_album', 'edit', $params, 'album_user_id');
	...
	public function actionDelete()
	{
		...
		if ($this->isConfirmedPost())
		{
			...
			$this->_sendAuthorAlert($album, 'xengallery_album', 'delete', $params, 'album_user_id');
	...
	{
		$albumId = $this->_input->filterSingle('album_id', XenForo_Input::UINT);
		$markAlbumId = $this->_input->filterSingle('mark_album_id', XenForo_Input::UINT);
		...
		if ($this->isConfirmedPost())
		{
			...
			if ($markAlbumId)
			{
				$fetchOptions['album_id'] = $markAlbumId;
			}
