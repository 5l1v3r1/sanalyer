...
class XenGallery_Install
{
	...
	public static function stepVersions()
	{
		...
		if ($version < 901010270)
		{
			XenForo_Application::defer('XenGallery_Deferred_UserMediaQuota', array('resetField' => true), 'XFMGQuotaRebuild', true, time() + 1);
		}
		else if ($version < 901010370)
		{
			// Running this again in 1.1.3 just in case - some users may have been skipped before in an edge case with a miscounted media_count field.
			XenForo_Application::defer('XenGallery_Deferred_UserMediaQuota', array('resetField' => true), 'XFMGQuotaRebuild', true, time() + 1);
		}

		if ($version < 901010470)
		{
			$db->update('xf_option', array(
				'option_value' => 10
		...
		if ($version < 901010570)
		{
			self::_executeQuery("
				...
				VALUES
					('xengalleryAllowUploadFromUrl', '1', '', '', '', '', '',
					1, '', '', 'XenGallery')
			");
	...
	protected static function _getData()
	{
		...
		if (!self::$_version)
		{
			...
			$data['xf_admin_permission_entry'] = "
				...
				SELECT user_id, 'manageXenGallery'
