...
class XenGallery_Model_Media extends XenForo_Model
{
	...
	public function rebuildUserMediaQuota(array $userIds)
	{
		...
		foreach ($userIds AS $userId)
		{
			...
			$db->update('xf_user', array('xengallery_media_quota' => $diskUsage ? round($diskUsage / 1024) : 0), 'user_id = ' . $db->quote($userId));
		}
	...
	public function canTagMedia(array $media, &$errorPhraseKey = '', array $viewingUser = null)
	{
		...
		if ($tagLimit !== 0 && !empty($media['userTags']))
		{
			if ($tagLimit != -1)
			{
				$tagsByUser = 0;
				foreach ($media['userTags'] AS $tag)
				{
	...
	*/
	...
	public function requiresTranscoding($data)
	{
		...
		/*if ($ffmpegPath = $this->getFfmpegPath())
		{
			$video = new XenGallery_Helper_Video($filePath);
			$result = new XenGallery_VideoInfo_Result($video->getVideoCodecs());
		}
		else
		{
	...
	{
		return XenForo_Application::getOptions()->get('xengalleryVideoTranscoding', 'ffmpegPath');
	}
	...
	public function isValidVideo($tempFile)
	{
