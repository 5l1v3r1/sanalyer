...
class XenGallery_Helper_Video
{
	...
	protected $_actualAacEncoders = array();
	...
	protected function _assertFfmpegIsValid(array $output)
	{
		...
		foreach ($output AS $line)
		{
			...
			if (preg_match('/(?:[a-z]|\.){6}\s(libvo_aacenc|aac|libx264|png)\s/im', $line, $matches))
			{
		...
		if ($encoders !== null)
		{
			...
			else
			{
				...
				$validAacEncoders = array('aac', 'libvo_aacenc');
				$this->_actualAacEncoders = array_intersect($available, $validAacEncoders);
				if (!$this->_actualAacEncoders)
				{
					$this->_ffmpegEncoderError = new XenForo_Phrase('xengallery_xfmg_requires_one_of_following_aac_encoders_to_be_enabled', array('aacEncoders' => implode(', ', $validAacEncoders)));
					return false;
				}
			}
		}
	...
	public function getVideoCodecs()
	{
		...
		{
			$line = trim($line);
			if ($line && preg_match('/Stream\s+#\d+:\d+(?:.*):\s+(Video|Audio):\s+(.{3,4})\s+/sU', $line, $match))
			{
				if (isset($match[1], $match[2]))
				{
					switch ($match[1])
					{
						case 'Audio':
							...
							$result['audioCodec'] = $match[2];
							break;

						case 'Video':
							$result['hasVideo'] = true;
							$result['videoCodec'] = $match[2];
							break;
					}
				}
			}
