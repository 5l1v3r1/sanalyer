...
class XenGallery_Importer_vBulletin42x extends XenForo_Importer_Abstract
{
	...
	{
		return ' XFMG: Import From vBulletin 4.2 Albums';
	}
	...
	public function configure(XenForo_ControllerAdmin_Abstract $controller, array &$config)
	{
		if ($config)
		{
			...
			');
			if ($settings['attachfile'] !== self::ATTACH_AS_DB)
			{
				return $controller->responseView('XenGallery_ViewAdmin_Import_Config', 'xengallery_import_config_vbulletin', array(
					...
					'albumPicPath' => $settings['attachpath'],
					'attachType' => $settings['attachfile'],
	...
	public function stepMedia($start, array $options)
	{
		...
		foreach ($media AS $item)
		{
			...
			$xengalleryMedia = array(
				'media_title' => $item['caption'] ? utf8_substr($this->_convertToUtf8($item['caption'], true), 0, $titleMaxLength) : utf8_substr($this->_convertToUtf8($item['filename'], true), 0, $titleMaxLength),
			...
			$xfAttachmentData = array(
				...
				'filename' => $this->_convertToUtf8($item['filename']),
				'attach_count' => 1
			);
	...
	public function stepComments($start, array $options)
	{
		...
		$comments = $sDb->fetchAll($sDb->limit(
				...
				LEFT JOIN ' . $prefix . 'filedata AS filedata ON
					(comment.filedataid = filedata.filedataid)
				LEFT JOIN ' . $prefix . 'attachment AS attachment ON
					(filedata.filedataid = attachment.filedataid)
		...
		$this->_userIdMap = $this->_importModel->getUserIdsMapFromArray($comments, 'postuserid');
		foreach ($comments AS $comment)
		{
			$next = $comment['commentid'];
			if (!isset($this->_userIdMap[$comment['postuserid']]))
			{
				continue;
			}
	...
	protected function _importComment(array $comment, array $options)
	{
		...
		$mediaId = $model->mapMediaId($comment['attachmentid']);
		if (!$mediaId)
		{
			return false;
		}
