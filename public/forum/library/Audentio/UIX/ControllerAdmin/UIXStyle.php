...
class Audentio_UIX_ControllerAdmin_UIXStyle extends XenForo_ControllerAdmin_Abstract
{
    public function actionIndex()
    {
        ...
        $apiResponse = $audentioModel->getStylesFromApi();

        if ($apiResponse['status'] == 'error') {
            return $this->responseError($apiResponse['error']);
        }

        $styles = $apiResponse['styles'];
        #var_dump($styles);die;

        $availableUpdates = 0;

        foreach ($styles as &$style) {
            $installed = $styleModel->getAudentioStyleByPid($style['id']);
            if ($installed && $installed['uix_version'] != $style['latest_version'] && !$installed['uix_update_available']) {
    ...
    public function actionOutdated()
    {
        ...
        $styleResponse = $audentioModel->getStylesFromApi();
        if ($styleResponse['status'] == 'error') {
            return $this->responseError($styleResponse['error']);
        }

        $styles = $styleResponse['styles'];

        foreach ($styles as &$style) {
            $style['installed'] = $styleModel->getAudentioStyleByPid($style['id']);
        }
    ...
    public function actionInstall()
    {
        ...
        if ($this->isConfirmedPost()) {
            ...
            $apiResponse = $audentioModel->getStyleFromApi($pid);
            if ($apiResponse['status'] == 'error') {
                return $this->responseError($apiResponse['error']);
            }
            $product = $apiResponse['payload']['product'];
    ...
    {
        $audentioModel = $this->getModelFromCache('Audentio_UIX_Model_Audentio');
        $productResponse = $audentioModel->getStyleFromApi($pid);
        if ($productResponse['status'] == 'error') {
            return $this->responseError($productResponse['error']);
        }
        ...
        $apiResponse = $audentioModel->downloadStyleFromApi($pid);
        if ($apiResponse['status'] == 'error') {
            return $this->responseError($apiResponse['error']);
        }
    ...
    public function actionInstallStep2()
    {
        ...
        $xmlFile = $dirPath.DIRECTORY_SEPARATOR.'style-'.str_replace(' ', '-', $product['product_name']).'.xml';
        ...
        $input['version'] = $uixSession['version'];
