...
class Audentio_UIX_Model_Node extends XFCP_Audentio_UIX_Model_Node
{
    ...
    {
        $nodes = $this->getAllNodesWithFields();
        $nodeStylingCache = array();

        foreach ($nodes as $node) {
            $nodeStylingCache[$node['node_id']] = @unserialize($node['uix_styling']);
        }

        $this->_getDataRegistryModel()->set('uix_nodeStylingCache', $nodeStylingCache);
        $this->rebuildUixNodeCache();
    }
    ...
    public function rebuildNodeClassCache()
    {
        ...
        foreach ($nodes as $node) {
			...
			if ($node['inheritClass'] && $node['parent_node_id']) {
				$parentNode = @unserialize($nodes[$node['parent_node_id']]['uix_styling']);
				if ($parentNode && array_key_exists('nodeInfo-class', $parentNode)) {
					$nodeClassCache[$node['node_id']] = (!empty($node['uix_styling']['nodeInfo-class']) ? $node['uix_styling']['nodeInfo-class'] . ' ' : '') . $parentNode['nodeInfo-class'];
				}
			}
        }
    ...
    {
        return $this->_getDataRegistryModel()->get('uix_nodeStylingCache');
    }
