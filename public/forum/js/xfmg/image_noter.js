...
!function($, window, document, _undefined)
{
	...
	XFMG.ImageNoter = XF.Element.newHandler({
		...
		init: function()
		{
			...
			{
				$image.on('load', $.proxy(this, 'prepareNotes'));
			}
			else
			{
				this.prepareNotes();
			...
			$(window).onPassive('resize', $.proxy(this, 'prepareNotes'));

			// no toggle == no permission to add notes
			if ($toggle.length)
			{
				this.$toggle = $toggle;
				$toggle.on('click', $.proxy(this, 'toggleNewNoteEditor'));
			}
		},
		...
		initNote: function($note)
		{
			...
			tooltip.addSetupCallback(function($tooltipEl)
			{
				...
				$tooltipEl.find('.js-mediaNoteTooltipEdit').on('click', $.proxy(self, 'editNote', $note));
			});
		...
		editNote: function($note)
		{
			...
			this.$image.cropper({
				...
				ready: function()
				{
					...
					{
						self.tooltip = new XF.TooltipElement($.proxy(self, 'getEditNoteTooltipContent'), {
				...
				cropstart: $.proxy(this, 'editNoteCropstart'),
				cropend: $.proxy(this, 'editNoteCropend')
			});
		},
		...
		editNoteLoaded: function(data, onContent)
		{
			...
			XF.setupHtmlInsert(data.html, function($html, container, onComplete)
			{
				...
				$cancel.on('click', $.proxy(self, 'editNoteCancel'));

				$html.on('ajax-submit:response', $.proxy(self, 'editNoteHandle'));
			});
		},
		...
		enableNewNoteEditor: function()
		{
			...
			this.$image.cropper({
				...
				zoomable: false,
				toggleDragModeOnDblclick: false,
				ready: $.proxy(this, 'newNoteReady'),
				cropstart: $.proxy(this, 'newNoteCropstart'),
				cropend: $.proxy(this, 'newNoteCropend')
			});
		},
		...
		newNoteCropend: function(e)
		{
			...
			{
				this.tooltip = new XF.TooltipElement($.proxy(this, 'getNewNoteTooltipContent'), {
					extraClass: 'tooltip--mediaNote',
					html: true,
					loadRequired: true
				});
		...
		newNoteLoaded: function(data, onContent)
		{
			...
			XF.setupHtmlInsert(data.html, function($html, container, onComplete)
			{
				...
				var $cancel = $html.find('.js-cancelButton');
				$cancel.on('click', $.proxy(self, 'newNoteCancel'));

				$html.on('ajax-submit:response', $.proxy(self, 'newNoteHandle'));
			});
		},
