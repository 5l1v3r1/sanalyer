	...
	XFMG.ImageEditor = XF.Element.newHandler({
		...
		init: function()
		{
			...
			$image.cropper({
				...
				ready: function()
				{
					$form.find(self.options.move + ', ' + self.options.crop).on('click', $.proxy(self, 'dragMode'));
					$form.find(self.options.zoomIn + ', ' + self.options.zoomOut).on('click', $.proxy(self, 'zoom'));
					$form.find(self.options.rotateLeft + ', ' + self.options.rotateRight).on('click', $.proxy(self, 'rotate'));
					$form.find(self.options.flipH + ', ' + self.options.flipV).on('click', $.proxy(self, 'flip'));

					$form.find(self.options.clear).on('click', $.proxy(self, 'clear'));
				},
				crop: function(e)
				{
					var json = {
