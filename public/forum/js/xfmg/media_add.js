...
!function($, window, document, _undefined)
{
	...
	XFMG.MediaManager = XF.Element.newHandler({
		...
		init: function()
		{
			...
			$filesContainer.on('click', this.options.actionButton, $.proxy(this, 'actionButtonClick'));
		},
	...
	XFMG.LinkChecker = XF.Element.newHandler({
		...
		init: function()
		{
			...
			this.$pasteInput
				.on('paste', $.proxy(this, 'paste'));

			$target.on('ajax-submit:response', $.proxy(this, 'complete'));
			$target.on('ajax-submit:error', $.proxy(this, 'error'));
		},
